...
from holla import ChallahArithExpr, ChallahArray, ChallahBranch, ChallahLeaf, ChallahTree, ChallahVar
...
def balanced_reduce(values: list[bit], function: Callable[[bit, bit], bit]) -> bit:
    if len(values) == 0:
        raise Exception('reducing empty list!')
    if len(values) == 1:
        return values[0]
    mid = len(values) // 2
...
def lt_circuit(left: num, right: num) -> bit:
    ...
    return balanced_reduce([eq_bit * lt_bit for eq_bit, lt_bit in zip(running_eq[1:], lt_bits)], lambda a, b: a * b + a + b)  # OR the partial comparisons together!


def mux(condition: bit, true: num, false: num) -> num:
    def mux_bit(condition: bit, true: bit, false: bit): return condition * true + (coyote_ast.Var('1') + condition) * false
